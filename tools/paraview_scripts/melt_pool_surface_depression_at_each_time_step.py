...
def main():
    ...
    foamfoam.CellArrays = ['alpha.metal']
    foamfoam.UpdatePipelineInformation()
    time_steps = foamfoam.TimestepValues
    UpdatePipeline(time=time_steps[0], proxy=foamfoam)
    data = {}

    for time in time_steps:
        
        height  = get_surface_depression_at_laser_point(case_directory,foamfoam,'alpha.metal', time)
        data[time] = height

    with open('depression_data.csv', 'w') as f:
        w = csv.writer(f)
